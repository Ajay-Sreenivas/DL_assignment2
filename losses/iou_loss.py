...
class CombinedLocLoss(nn.Module):
    """Combined IoU + SmoothL1 localisation loss.

    Pure IoU loss has two failure modes:
      1. When predicted and target boxes do not overlap, the IoU gradient is
         exactly zero — the model receives no signal on how far off it is.
      2. IoU is scale-invariant, which can destabilise early training.

    Adding a SmoothL1 term fixes both:
    ...
    Loss = IoU_loss + λ × SmoothL1_loss
    Both terms are computed in the same coordinate space (pixel space).
